...
class VitsGeneratorLossLossWeight(nn.Module):
    def __init__(self, c: Coqpit):
        super().__init__()
        self.kl_loss_alpha = c.kl_loss_alpha
        self.gen_loss_alpha = c.gen_loss_alpha
    ...
    @staticmethod
    def feature_loss(feats_real, feats_generated, loss_weight):
        loss = 0
        for dr, dg in zip(feats_real, feats_generated):
            for rl, gl in zip(dr, dg):
                rl = rl.float().detach()
    ...
    @staticmethod
    def generator_loss(scores_fake, loss_weight):
        loss = 0
        gen_losses = []
        for dg in scores_fake:
            dg = dg.float()
    ...
    @staticmethod
    def kl_loss(z_p, logs_q, m_p, logs_p, z_mask, loss_weight):
        """
        z_p, logs_q: [b, h, t_t]
        m_p, logs_p: [b, h, t_t]
        """
    ...
    def forward(
        ...
        Shapes:
            ...
            - feats_disc_real[i][j]: :math:`[B, C, T', P]`
        """
        # print(f'loss_weight in loss forward: {loss_weight}')
        loss = 0.0
        return_dict = {}
        z_mask = sequence_mask(z_len).float()
        # compute losses
        loss_kl = (
            self.kl_loss(z_p=z_p, logs_q=logs_q, m_p=m_p, logs_p=logs_p, z_mask=z_mask.unsqueeze(1), loss_weight=loss_weight)
            * self.kl_loss_alpha
        )
        loss_feat = (
            self.feature_loss(feats_real=feats_disc_real, feats_generated=feats_disc_fake, loss_weight=loss_weight) * self.feat_loss_alpha
        )
        loss_gen = self.generator_loss(scores_fake=scores_disc_fake, loss_weight=loss_weight)[0] * self.gen_loss_alpha
        loss_mel = (torch.nn.functional.l1_loss(mel_slice, mel_slice_hat, reduction="none").transpose(0, -1) * loss_weight).mean() * self.mel_loss_alpha
        # loss_duration = torch.sum(loss_duration.float()) * self.dur_loss_alpha
        loss_duration = torch.sum(loss_duration.float().transpose(0, -1) * loss_weight) * self.dur_loss_alpha 
        loss = loss_kl + loss_feat + loss_mel + loss_gen + loss_duration
        ...
        # print(f'loss_gen.shape: {loss_gen.shape}')
        # print(f'loss_kl.shape: {loss_kl.shape}')
        # print(f'loss_feat.shape: {loss_feat.shape}')
        # print(f'loss_mel.shape: {loss_mel.shape}')
        # print(f'loss_duration.shape: {loss_duration.shape}')
...
class VitsDiscriminatorLossLossWeight(nn.Module):
    def __init__(self, c: Coqpit):
        super().__init__()
        self.disc_loss_alpha = c.disc_loss_alpha

    @staticmethod
    def discriminator_loss(scores_real, scores_fake, loss_weight):
        loss = 0
        real_losses = []
        fake_losses = []
        for dr, dg in zip(scores_real, scores_fake):
            dr = dr.float()
            dg = dg.float()
            real_loss = torch.mean(((1 - dr) ** 2).transpose(0, -1) * loss_weight)
            fake_loss = torch.mean((dg**2).transpose(0, -1) * loss_weight)
            loss += real_loss + fake_loss
            real_losses.append(real_loss.item())
            fake_losses.append(fake_loss.item())
        return loss, real_losses, fake_losses

    def forward(self, scores_disc_real, scores_disc_fake, loss_weight):
        loss = 0.0
        return_dict = {}
        loss_disc, loss_disc_real, _ = self.discriminator_loss(
            scores_real=scores_disc_real, scores_fake=scores_disc_fake, loss_weight=loss_weight
        )
        return_dict["loss_disc"] = loss_disc * self.disc_loss_alpha
        loss = loss + return_dict["loss_disc"]
        return_dict["loss"] = loss
