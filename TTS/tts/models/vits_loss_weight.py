...
class VitsLossWeightDataset(TTSDataset):
    ...
    def __getitem__(self, idx):
        ...
        raw_text = item["text"]
        # print(f'item in __getitem__: {item}')
    ...
    def collate_fn(self, batch):
        ...
        return {
            ...
            "loss_weight": torch.tensor(batch["loss_weight"], dtype=torch.float),
        }
...
class Vits(BaseTTS):
    ...
    def train_step(self, batch: dict, criterion: nn.Module, optimizer_idx: int) -> Tuple[Dict, Dict]:
        ...
        # print(f'batch in train_step: {batch}')
    ...
    def format_batch(self, batch: Dict) -> Dict:
        ...
        if self.speaker_manager is not None and self.speaker_manager.embeddings and self.args.use_d_vector_file:
            ...
            d_vectors = [d_vector_mapping[f"{w.replace('train_16k', 'train')}"]["embedding"] for w in batch["audio_unique_names"]]
            # SSB00380297 vs aishell3_sub#train/wav/SSB0339/SSB03390076
            # d_vectors = []
            # for w in batch["audio_unique_names"]:
            #     try:
            #         d_vector = d_vector_mapping[f"{w}"]["embedding"]
            #     except KeyError:
            #         d_vector = d_vector_mapping[f"{w}".split('/')[-1]]["embedding"]
    ...
    def get_criterion(self):
        ...
        from TTS.tts.layers.losses import (  # pylint: disable=import-outside-toplevel
            ...
            # VitsGeneratorLoss,
            VitsDiscriminatorLossLossWeight,
            VitsGeneratorLossLossWeight,
        )

        return [VitsDiscriminatorLossLossWeight(self.config), VitsGeneratorLossLossWeight(self.config)]
